...
public class UIStyleManager implements CustomizationManager<UIStyleOption> {

    private static UIStyleManager sUIStyleOptionManager;
    private Context mContext;
    private UIStyleOption mActiveOption;
    ...
    @Override
    public void apply(UIStyleOption option, @Nullable Callback callback) {
        if (!persistOverlay(option)) {
            Toast failed = Toast.makeText(mContext, "Failed to apply UI Style, reboot to try again.", Toast.LENGTH_SHORT);
            failed.show();
            if (callback != null) {
    ...
    @Override
    public void fetchOptions(OptionsFetchedListener<UIStyleOption> callback, boolean reload) {
        List<UIStyleOption> options = mProvider.getOptions();
        for (UIStyleOption option : options) {
    ...
    private boolean persistOverlay(UIStyleOption toPersist) {
    ...
    public static UIStyleManager getInstance(Context context, OverlayManagerCompat overlayManager) {
        if (sUIStyleOptionManager == null) {
            Context applicationContext = context.getApplicationContext();
            sUIStyleOptionManager = new UIStyleManager(context, overlayManager, new UIStyleOptionProvider(applicationContext, overlayManager));
        }
        return sUIStyleOptionManager;
    }
